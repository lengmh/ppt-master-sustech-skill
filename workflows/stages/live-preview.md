...
- **Optional browser geometry probe**: run `scripts/svg_layout_probe.py` explicitly when static layout warnings are ambiguous or before a high-stakes export. It uses the same `/api/slide/<page>` materialization path as live preview, so relative images and `<use data-icon>` content match the browser view. The probe writes diagnostic JSON only; it does not stage edits, write annotations, mutate SVG/PPTX/spec files, or replace the live-preview UI.
