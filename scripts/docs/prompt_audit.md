...
```bash
uv run --with 'tiktoken>=0.7.0' python scripts/prompt_audit.py        # text summary
uv run --with 'tiktoken>=0.7.0' python scripts/prompt_audit.py --json # stable JSON report
```

`tiktoken` remains an ephemeral maintainer-tool dependency and is not added to
`requirements.txt`, `pyproject.toml`, or `uv.lock`. When the current HTTP proxy
uses SOCKS, add `--with 'pysocks>=1.7.1'` to the same command.
