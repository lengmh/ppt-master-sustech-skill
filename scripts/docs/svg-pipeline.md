...
python3 scripts/svg_quality_checker.py examples/project --layout-json <output.json>
...
- conservative layout-quality diagnostics for text overflow, suspicious font size, card overlap, and declared text-budget mismatch
...
Layout-quality findings remain advisory and never reflow text, resize cards,
split pages, shrink fonts, or mutate SVG/PPTX/spec files. Ambiguous static
findings may set `probe_recommended=true` in `--layout-json` output.

### Optional browser geometry probe

Run the explicit browser probe when static findings are ambiguous or before a
high-stakes export:

```bash
uv run --with playwright python scripts/svg_layout_probe.py <project_path> --pages 03 --output <output.json>
```

Project mode uses the live-preview server's `/api/slide/<page>` materialized
SVG, so relative images and `<use data-icon>` content match the browser view.
The probe writes diagnostics only; it does not stage edits, write annotations,
or mutate SVG/PPTX/spec files.
