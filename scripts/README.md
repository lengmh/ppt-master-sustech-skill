...
python3 scripts/package_release.py --temp-root /home/sustech/TempFiles --keep-temp
```
...
Notes:
- `update_repo.py` prefers the skill-local `requirements.txt` and falls back to repo-root `requirements.txt` only if the local file is absent.
- `package_release.py` reads `VERSION` + `RELEASE_META.json`, expects local `.env.example` + `requirements.txt` by default, and emits `ppt-master-sustech-r<upstream>-v<local>-YYYY-MM-DD.zip` unless `--output` overrides it.
