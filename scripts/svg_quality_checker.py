...
def print_usage() -> None:
    ...
    print("  --layout-json <path>           Export static layout-quality warnings as JSON")
...
def main() -> None:
    ...
    if '--layout-json' in sys.argv:
        idx = sys.argv.index('--layout-json')
        if idx + 1 >= len(sys.argv):
            print("[ERROR] --layout-json requires a path")
            sys.exit(1)
        checker.export_layout_json(sys.argv[idx + 1])
